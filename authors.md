# Authors and Citation

## Authors

- **Léopold Guyot**. Author, maintainer.
  [](https://orcid.org/0009-0005-2217-3855)

- **Loïc Guille**. Author. [](https://orcid.org/0000-0002-8387-1092)

## Citation

Source:
[`DESCRIPTION`](https://github.com/UCLouvain-CBIO/QFeaturesGUI/DESCRIPTION)

Guyot L, Guille L (2026). *QFeaturesGUI: A suite of shiny apps to use
the main functionalies of the QFeatures package*. R package version
0.1.1, <https://uclouvain-cbio.github.io/QFeaturesGUI/>.

    @Manual{,
      title = {QFeaturesGUI: A suite of shiny apps to use the main functionalies of the QFeatures package},
      author = {Léopold Guyot and Loïc Guille},
      year = {2026},
      note = {R package version 0.1.1},
      url = {https://uclouvain-cbio.github.io/QFeaturesGUI/},
    }
